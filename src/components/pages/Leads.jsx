...
import { Plus, Search, Filter, Edit, Trash2, Users, Calendar, DollarSign, TrendingUp, Check, RefreshCw, X, Tag, Settings, AlertTriangle, User, Clock, CalendarDays, Download, ChevronLeft, ChevronRight, UserPlus, Bell } from 'lucide-react'
...
export default function Leads() {
  ...
  const [tagFilterSearchTerm, setTagFilterSearchTerm] = useState('')
  ...
  const [lembreteLeadId, setLembreteLeadId] = useState(null)
  ...
  const uniqueAltered = useMemo(() => {
    const altered = new Set()
    leads.forEach(lead => {
      if (lead.alterado_por_nome) {
        altered.add(lead.alterado_por_nome)
      }
    })
    return Array.from(altered).sort()
  }, [leads])
  ...
  const handleAbrirLembrete = useCallback((leadId) => {
    setLembreteLeadId(leadId)
    setLembreteForm({
      data_lembrete: '',
      descricao: '',
      observacoes: ''
  ...
  const handleSalvarLembrete = useCallback(async () => {
    try {
      if (!lembreteForm.data_lembrete || !lembreteForm.descricao) {
        alert('Por favor, preencha a data e a descrição do lembrete')
        return
      }
  ...
  }, [lembreteForm, lembreteLeadId, user])
  ...
  // Abrir lead automaticamente quando vindo da tela de Lembretes
  useEffect(() => {
    if (location.state?.openLeadId && leads.length > 0) {
      const leadToOpen = leads.find(lead => lead.id === location.state.openLeadId)
      if (leadToOpen) {
        handleEdit(leadToOpen)
        // Limpar o state para não reabrir ao recarregar
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {alteredByFilter !== 'Todos' && (
              <div className="flex items-center gap-2 text-sm text-purple-600 bg-purple-50 px-3 py-1 rounded-lg border border-purple-200">
                ...
                <span>Alterado por: {alteredByFilter}</span>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-5 w-5 p-0 hover:bg-purple-200"
                  onClick={() => setAlteredByFilter('Todos')}
                >
                  <X className="h-3 w-3" />
                </Button>
              </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  {selectedTagsFilter.length > 0 && (
                    ...
                    >
                      Limpar ({selectedTagsFilter.length})
                    </button>
                  )}
                </div>
              </CardHeader>
              <CardContent>
                {/* Campo de busca de tags no filtro */}
                <div className="relative mb-3">
                  <Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
                  <Input
                    placeholder="Buscar tags..."
                    value={tagFilterSearchTerm}
                    onChange={(e) => setTagFilterSearchTerm(e.target.value)}
                    className="pl-9 text-sm bg-white"
                  />
                  {tagFilterSearchTerm && (
                    <button
                      type="button"
                      onClick={() => setTagFilterSearchTerm('')}
                      className="absolute right-2 top-2.5 text-gray-400 hover:text-gray-600"
                    >
                      <X className="h-4 w-4" />
                    </button>
                ...
                <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto p-2 bg-white rounded-lg border border-blue-200">
                  {tags
                    .filter(tag => {
                        ...
                        <tr key={lead.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-4">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleAbrirLembrete(lead.id)}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setLembreteForm(prev => ({ ...prev, data_lembrete: e.target.value }))}
                required
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setLembreteForm(prev => ({ ...prev, descricao: e.target.value }))}
                required
              />
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                value={lembreteForm.observacoes}
                onChange={(e) => setLembreteForm(prev => ({ ...prev, observacoes: e.target.value }))}
                rows={3}
              />
            </div>
            ...
            <div className="flex justify-end gap-2">
              <Button
                variant="outline"
                onClick={() => setIsLembreteDialogOpen(false)}
              >
                Cancelar
              </Button>
              <Button onClick={handleSalvarLembrete}>
                Salvar Lembrete
              </Button>
            </div>
          </div>
      ...
      {showKanban && (
        <FunilKanban
          ...
          tags={tags}
          onUpdateLead={handleUpdateLeadFromKanban}
